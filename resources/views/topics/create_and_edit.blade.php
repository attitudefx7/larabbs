    ...
    <script>
        $(document).ready(function(){
            var editor = new Simditor({
                textarea: $('#editor'),
                upload:{
                    url:'{{route('topics.upload_image')}}', // 处理上传图片的url
                    params:{_token:'{{csrf_token()}}'}, // 表单提交参数
                    fileKey:'upload_file', // 服务端获取图片的键值
                    connectionCount:3, // 最多只能上传3张图片
                    leaveConfirm:'文件上传中，关闭此页面将取消上传。' // 上传过程中，用户关闭页面时的提示
                },
                pasteImage:true, // 是否支持粘贴图片上传
            });
        });
    </script>
